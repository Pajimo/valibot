...
- Change `minBytes`, `notBytes`, `minCodePoints`, `maxCodePoints`, `notCodePoints`, `minGraphemes`, `maxGraphemes`, `notGraphemes`, `minWords`, `maxWords` and `notWords` actions to stop counting early or skip counting entirely when the string length already decides the result to improve performance (pull request #1631)
